...
// Statable : we don't need to take care of Statable whether it is list or not
template <typename Program, typename Drawable, typename... Statable>
auto render(Program const &program, Drawable const &drawable,
	    Statable... const &statables) -> bool {
  auto res = conf(statables, program) && ...;
  use(program);
  draw(drawable);
  return res;
}
